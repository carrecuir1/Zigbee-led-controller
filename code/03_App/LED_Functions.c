...
extern TIM_HandleTypeDef htimpwm_leds;
...
void Set_Single_LED_Brightness(int i, uint8_t brightness);
...
void Set_LED(int LED_Number, uint8_t red, uint8_t green, uint8_t blue, float brightness_angle) {
	LED_Data[LED_Number][0] = LED_Number;
	LED_Data[LED_Number][1] = green / tan(brightness_angle);
	LED_Data[LED_Number][2] = red / tan(brightness_angle);
	LED_Data[LED_Number][3] = blue / tan(brightness_angle);
}
...
void Set_Single_LED_Brightness(int i, uint8_t brightness) {
	for (int j = 1; j < 4; j++)				//Loop from 1 to 4 for colors
		...
		float angle = 90 - brightness;	//Measure the angle of brightness
		angle = angle * PI / 180;			//Convert from degrees to radian
		LED_Data[i][j] = (LED_Data[i][j]) / tan(angle);//Set the modified array the same as the data array but with a new RGB data
	}
}
...
void Send_To_LEDS(void) {
	...
	for (int i = 0; i < AMOUNT_OF_LEDS; i++)			//loop through all LED's
			{
		color =
				((LED_Data[i][1] << 16) | (LED_Data[i][2] << 8) | (LED_Data[i][3]));//Set the MSB and LSB
	...
	HAL_StatusTypeDef status = HAL_TIM_PWM_Start_DMA(&htimpwm_leds, TIM_CHANNEL_1, (uint32_t*) pwmData, index);//Start timer and set the data
	while (!data_Sent_Flag && status == HAL_OK) {
...
void Rainbow_Effect(uint8_t brightness, int rainbow_Time) {
	...
	float angle = 90 - brightness;	//Measure the angle of brightness
	angle = angle * PI / 180;			//Convert from degrees to radian
	for(uint16_t j=0;j<AMOUNT_OF_LEDS;j++) {
	  ind = effect_Step + j * 1;
	  switch((int)((ind % 60) / 20)) {
		case 0: factor1 = 1.0 - ((float)(ind % 60 - 0 * 20) / 20);
				factor2 = (float)((int)(ind - 0) % 60) / 20;
				/************ chnaged here *********/
				Set_LED(j, 255 * factor1 + 0 * factor2, 0 * factor1 + 255 * factor2, 0 * factor1 + 0 * factor2, angle);
				// Set_Single_LED_Brightness(j, brightness);
				break;
		case 1: factor1 = 1.0 - ((float)(ind % 60 - 1 * 20) / 20);
				factor2 = (float)((int)(ind - 20) % 60) / 20;
				Set_LED(j, 0 * factor1 + 0 * factor2, 255 * factor1 + 0 * factor2, 0 * factor1 + 255 * factor2, angle);
				// Set_Single_LED_Brightness(j, brightness);
				break;
		case 2: factor1 = 1.0 - ((float)(ind % 60 - 2 * 20) / 20);
				factor2 = (float)((int)(ind - 40) % 60) / 20;
				Set_LED(j, 0 * factor1 + 255 * factor2, 0 * factor1 + 0 * factor2, 255 * factor1 + 0 * factor2, angle);
				// Set_Single_LED_Brightness(j, brightness);
				break;
	  }
	}
	Send_To_LEDS();
	if(effect_Step >= 60) {effect_Step=0; return 0x03; }
	else effect_Step++;
}
...
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim) {
	HAL_TIM_PWM_Stop_DMA(&htimpwm_leds, TIM_CHANNEL_1);
	data_Sent_Flag = 1;
}
